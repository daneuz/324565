...
data class Peer(val endpointId: String, val nodeId: String, val name: String, val isConnected: Boolean, val avatar: String = "👤")
data class Message(val id: String, val senderNodeId: String, val senderName: String, val text: String, val isMe: Boolean, val targetNodeId: String, val fileUri: String? = null, val senderAvatar: String = "👤")
...
class MeshNetworkManager(private val context: Context) {
    ...
    private val _myAvatar = MutableStateFlow(prefs.getString("my_avatar", "👤") ?: "👤")
    ...
    fun updateName(newName: String, newAvatar: String = _myAvatar.value) {
        if (newName.isNotBlank() && (_myName.value != newName.trim() || _myAvatar.value != newAvatar)) {
            _myName.value = newName.trim()
            _myAvatar.value = newAvatar
            prefs.edit().putString("my_name", _myName.value).putString("my_avatar", _myAvatar.value).apply()
            
            // Broadcast name change
            val messageId = UUID.randomUUID().toString()
            val rawData = "$messageId|$myNodeId|${_myName.value}|GLOBAL|RENAME|${_myName.value}|${_myAvatar.value}"
    ...
    private fun startAdvertising() {
        ...
        val endpointName = "$myNodeId|${_myName.value}|${_myAvatar.value}"
            ...
            payload.asBytes()?.let { bytes ->
                GlobalScope.launch(Dispatchers.IO) {
                    ...
                    if (parts.size == 6) {
                        ...
                        if (msgType == "RENAME") {
                            val newName = parts[5]
                            val newAvatar = if (parts.size >= 7) parts[6] else "👤"
                            _peers.update { current ->
                                val newMap = current.toMutableMap()
                                val peerEntry = newMap.entries.find { it.value.nodeId == senderNodeId }
                                if (peerEntry != null) {
                                    newMap[peerEntry.key] = peerEntry.value.copy(name = newName, avatar = newAvatar)
                                }
                                newMap
                            }
    ...
    private val connectionLifecycleCallback = object : ConnectionLifecycleCallback() {
        override fun onConnectionInitiated(endpointId: String, info: ConnectionInfo) {
            ...
            val nodeId = if (parts.isNotEmpty()) parts[0] else endpointId
            val nodeName = if (parts.size >= 2) parts[1] else info.endpointName
            val nodeAvatar = if (parts.size >= 3) parts[2] else "👤"
            
            Log.d(TAG, "Connection initiated: $nodeName")
            connectionsClient.acceptConnection(endpointId, payloadCallback)
            _peers.update { current ->
                val newMap = current.toMutableMap()
                newMap[endpointId] = Peer(endpointId, nodeId, nodeName, false, nodeAvatar)
                newMap
            }
        }
            ...
            if (result.status.statusCode == ConnectionsStatusCodes.STATUS_OK) {
                ...
                connectedNodeId?.let { nodeId ->
                    kotlinx.coroutines.GlobalScope.launch {
                        ...
                        sendSyncReq(myNodeId) // Sync private messages
                        sendSyncReq("GLOBAL") // Sync global messages
                    }
                }
            } else {
